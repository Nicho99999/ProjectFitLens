...
public class TricepExercises
{
    public List<TricepExercise> Exercises { get; set; } = new List<TricepExercise>();
    
    private readonly HttpClient _httpClient;

    public TricepExercises(HttpClient httpClient)
    {
        _httpClient = httpClient;
    }

    public async Task FetchTricepExercises()
    {
        var muscle = "triceps";
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            Exercises = JsonConvert.DeserializeObject<List<TricepExercise>>(result);
        }
        else
        {
