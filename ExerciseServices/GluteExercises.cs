...
public class GluteExercises
{
    public List<GluteExercise> Exercises { get; set; } = new List<GluteExercise>();
    
    private readonly HttpClient _httpClient;

    public GluteExercises(HttpClient httpClient)
    {
        _httpClient = httpClient;
    }

    public async Task FetchGluteExercises()
    {
        var muscle = "glutes";
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            Exercises = JsonConvert.DeserializeObject<List<GluteExercise>>(result);
        }
        else
        {
